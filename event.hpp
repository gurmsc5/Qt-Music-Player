...
private:
  // TODO
	//event types enum
	enum _eventtype { _TempoEvent, _NoteEvent, _EndTrackEvent } _EventType;
	//union to cast a different MIDI event
	union Event
	{
		TempoEvent _tempoEvent;
		NoteEvent _noteEvent;
		EndTrackEvent _EndTrackEvent;
		Event() : _EndTrackEvent() {};
	} event;
